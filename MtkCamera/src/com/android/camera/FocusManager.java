...
public class FocusManager implements CameraActivity.OnOrientationListener,
    ...
    private static final int[] MATRIX_FOCUS_MODE_DEFAULT_ARRAY = new int[] {
        ...
        R.array.pref_camera_focusmode_default_array, // photo stereo
        R.array.pref_video_focusmode_default_array, // video
        R.array.pref_video_focusmode_default_array, // video pip
        R.array.pref_video_focusmode_default_array, // video stereo
        ...
        if (mState == STATE_IDLE || mState == STATE_UNKNOWN) {
            ...
            if (mState == STATE_SUCCESS) {
                if (mParameters != null
                        ...
                        && "on".equals(mParameters.getDistanceMode()) && !faceExists) {
    ...
    /**
     * Whether multi zone af is supported or not.
     *
     * @return True if multi zone af is supported.
     */
    public boolean isMultiZoneAfSupported() {
        boolean isSupported = false;
        if (mParameters != null && "1".equals(mParameters.get(KEY_MZAF_SUPPORTED))) {
            isSupported = true;
        } else {
            isSupported = false;
        }
