...
public class ZoomManager extends ViewManager implements
    ...
    private float mLastZoomIndexFactor = ZERO;
    private String zoomText;
    private static final String DOUBLE_ZOOM = "1.5";
    ...
    public boolean onScroll(float dx, float dy, float totalX, float totalY) {
        Log.d(TAG, "onScroll(" + dx + ", " + dy + ", " + totalX + ", " + totalY + ")");
        return false;
    }
    ...
    public boolean onUp() {
        ...
        public boolean onDoubleTap(float x, float y) {
            if (!FeatureSwitcher.isSupportDoubleTapUp())
                return false;
            Log.d(TAG, "onDoubleTap(" + x + ", " + y + ") mZoomIndexFactor=" + mZoomIndexFactor
        ...
        public boolean onScale(float focusX, float focusY, float scale) {
            Log.d(TAG, "onScale(" + focusX + ", " + focusY + ", " + scale + ") mZoomIndexFactor="
                    + mZoomIndexFactor + ", isAppSupported()=" + isAppSupported()
                    + ", isEnabled()=" + isEnabled());
            // lifeifei@wind-mobi.com for double camera begin
            mLastZoomIndexFactor = mZoomIndexFactor;
            // lifeifei@wind-mobi.com for double camera end
            if (!isAppSupported() || !isEnabled()) {
                return false;
            }
            ...
            performZoom(zoomIndex, true);
            Log.d(TAG, "onScale() mZoomIndexFactor=" + mZoomIndexFactor);
            // lifeifei@wind-mobi.com for double camera begin
            if(WIND_DEF_DOUBLE_CAMERA_TEST) {
                int cameraId = mCameraActivity.getCameraId();
                if (scale > 1.0 && DOUBLE_ZOOM.equals(zoomText) && cameraId == 2) {
                    mCameraActivity.switchCamera2(0);
                    resetZoom();
                    performZoom(0, false);
                } else if (scale < 1.0 && mZoomIndexFactor == 1.0
                        && "1.0".equals(zoomText) && cameraId == 0) {
                    mCameraActivity.switchCamera2(2);
                    resetZoom();
                    performZoom(0, false);
                }
            }
        ...
        Log.d(TAG, "onScaleBegin(" + focusX + ", " + focusY + ")");
            return true;
        }
    ...
    public void resetZoom() {
        Log.d(TAG, "resetZoom() mZoomRatios=" + mZoomRatios
    ...
    private void performZoom(int zoomIndex, boolean userAction) {
        Log.d(TAG, "performZoom(" + zoomIndex + ", " + userAction
                + ", mDeviceSupport="
                + mDeviceSupport);
        ...
        if (userAction) {
            ...
            // lifeifei@wind-mobi.com for double camera begin
            zoomText = String.format(Locale.ENGLISH, PATTERN, zoomRation);
    ...
    public void setZoomParameter() {
        if (isAppSupported()) {
            ...
            if (mZoomRatios != null) {
                ...
                Log.d(TAG, "onCameraParameterReady() index = " + index + ", len = " + len
    ...
    private boolean isAppSupported() {
        ...
        Log.d(TAG, "isAppSupported() return " + enable);
        return enable;
    }
    ...
    private boolean isValidZoomIndex(int zoomIndex) {
        ...
        Log.d(TAG, "isValidZoomIndex(" + zoomIndex + ") return " + valid);
        return valid;
    }
