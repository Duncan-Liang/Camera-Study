...
// lifeifei@wind-mobi.com add 20170209 for double camera test begin
import android.os.SystemProperties;
// lifeifei@wind-mobi.com add 20170209 for double camera test end
...
public class PickerManager extends ViewManager implements Listener,
    ...
    public interface PickerListener {
        ...
        boolean onCameraPicked2(int camerId);
    ...
    // lifeifei@wind-mobi.com for double camera begin
    private PickerButton mCameraPicker2;
    // lifeifei@wind-mobi.com for double camera end
    ...
    // lifeifei@wind-mobi.com add 20170209 for double camera test begin
    private static final boolean WIND_DEF_DOUBLE_CAMERA_TEST = SystemProperties.get("ro.wind.def.double.camera.test").equals("1");
    // lifeifei@wind-mobi.com add 20170209 for double camera test end

    // lifeifei@wind-mobi.com for double camera begin
    private static final int PICKER_BUTTON_NUM = 6;
    // lifeifei@wind-mobi.com for double camera end
    private static final int BUTTON_HDR = 0;
    private static final int BUTTON_FLASH = 1;
    private static final int BUTTON_CAMERA = 2;
    // lifeifei@wind-mobi.com for double camera begin
    private static final int BUTTON_CAMERA_2 = 5;
    // lifeifei@wind-mobi.com for double camera end
    private static final int BUTTON_STEREO = 3;
    private static final int BUTTON_SLOW_MOTION = 4;
    private PickerButton[] mPickerButtons = new PickerButton[PICKER_BUTTON_NUM];

    // lifeifei@wind-mobi.com for double camera begin
    private static final int MAX_NUM_OF_SHOWEN = 5;
    private int[] mButtonPriority = { BUTTON_SLOW_MOTION, BUTTON_HDR, BUTTON_FLASH, BUTTON_CAMERA,
            BUTTON_STEREO,BUTTON_CAMERA_2 };
    // lifeifei@wind-mobi.com for double camera end
    ...
    static {
        ...
        sShownStatusRecorder[BUTTON_CAMERA_2] = false;
    }
    ...
    protected View getView() {
        ...
        // lifeifei@wind-mobi.com for double camera begin
        mCameraPicker2 = (PickerButton) view.findViewById(R.id.onscreen_camera_picker_2);
        // lifeifei@wind-mobi.com for double camera end
        ...
        // lifeifei@wind-mobi.com for double camera begin
        mPickerButtons[BUTTON_CAMERA_2] = mCameraPicker2;
        // lifeifei@wind-mobi.com for double camera end
    ...
    public void onCameraParameterReady() {
        ...
        // stereo camera have high priority,if the order of button is definite, do not
        // redefine again.
        if (!mDefineOrder) {
            int count = 0;
            for (int i = 0; i < mButtonPriority.length; i++) {
                ...
                case BUTTON_CAMERA:
                    ...
                    break;
                // lifeifei@wind-mobi.com for double camera begin
                case BUTTON_CAMERA_2:
                    pref = (IconListPreference) getContext().getListPreference(
                            SettingConstants.ROW_SETTING_DUAL_CAMERA_2);
                    if(WIND_DEF_DOUBLE_CAMERA_TEST){
                        visible = ModeChecker.getCameraPickerVisible(getContext());
                    }else {
                        visible = false;
                    }
    ...
    public boolean onPicked(PickerButton button, ListPreference pref, String newValue) {
        ...
        if (mListener != null) {
            ...
            // lifeifei@wind-mobi.com for double camera begin
            case BUTTON_CAMERA_2:
                picked = mListener.onCameraPicked2(Integer.parseInt(newValue));
                break;
            // lifeifei@wind-mobi.com for double camera end
            case BUTTON_STEREO:
    ...
    public void setCameraId(int cameraId) {
        if (mCameraPicker != null) {
            // lifeifei@wind-mobi.com for double camera begin
            if(cameraId == 2) {
                mCameraPicker.setValue("" + 0);
            }else{
                mCameraPicker.setValue("" + cameraId);
            }
        }
        if (mCameraPicker2 != null) {
            if(cameraId == 1) {
                mCameraPicker2.setValue("" + 0);
            }else{
                mCameraPicker2.setValue("" + cameraId);
            }
        }
            // lifeifei@wind-mobi.com for double camera end
    }
    ...
    public void onRefresh() {
        ...
        mCameraPicker.initialize((IconListPreference) getContext().getListPreference(
                SettingConstants.ROW_SETTING_DUAL_CAMERA));
        // lifeifei@wind-mobi.com for double camera begin
        mCameraPicker2.initialize((IconListPreference) getContext().getListPreference(
                SettingConstants.ROW_SETTING_DUAL_CAMERA_2));
        // lifeifei@wind-mobi.com for double camera end
        ...
        if (isCameraPickerVisible) {
            ...
            // lifeifei@wind-mobi.com for double camera begin
            if(WIND_DEF_DOUBLE_CAMERA_TEST) {
                mCameraPicker2.refresh();
            }else {
                mCameraPicker2.setVisibility(View.GONE);
            }
            // lifeifei@wind-mobi.com for double camera end
        } else {
